...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 p-6 md:p-8 rounded-3xl text-white shadow-xl shadow-slate-200/50 relative overflow-hidden">
        ...
        <div class="absolute -bottom-12 right-1/3 w-64 h-64 bg-purple-500/20 rounded-full blur-3xl pointer-events-none"></div>
        ...
        <div class="relative z-10 space-y-1">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-500/30 text-indigo-300 text-xs font-semibold mb-1 backdrop-blur-md">
                ...
                <span>Fasilitas & Infrastruktur Mitra (PT. Champ)</span>
            </div>
            <h1 class="text-2xl md:text-3xl font-extrabold tracking-tight">Manajemen Gudang Mitra</h1>
            <p class="text-slate-300 text-sm max-w-xl">Pantau dan kelola lokasi serta kapasitas penyimpanan logistik Mitra PT. Champ.</p>
        </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-100/60 overflow-hidden flex flex-col h-full">
                <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="font-bold text-lg text-slate-800">Daftar Inventaris Gudang Mitra</h2>
                            <p class="text-xs text-slate-400">Kapasitas & lokasi gudang operasional Mitra</p>
                        </div>
                    </div>
                </div>
                            ...
                            @forelse($gudangs as $gudang)
                                <tr class="hover:bg-slate-50/80 transition-all group">
                                    <td class="px-6 py-5">
                                        ...
                                        <p class="text-[11px] text-indigo-600 font-semibold mt-0.5">👤 Mitra: {{ $gudang->user->name ?? 'Belum Diketahui' }}</p>
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        ...
                                        <div class="flex justify-center items-center gap-1.5">
                                            <a href="{{ route('mitra-gudang.edit', $gudang) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                            </a>
                                            <form action="{{ route('mitra-gudang.destroy', $gudang) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus gudang mitra ini?')">
                                                @csrf @method('DELETE')
                                        ...
                                        @else
                                            <span class="text-xs text-slate-400 font-medium">Biasa</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="py-16 text-center text-slate-400">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-sm font-medium">Belum ada data gudang mitra yang tersimpan.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
        ...
        <div class="lg:col-span-4 bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-100/60 overflow-hidden flex flex-col min-h-[500px]">
            <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400">Peta distribusi lokasi gudang mitra</p>
                    </div>
                </div>
            ...
            <div class="flex-1 relative bg-slate-50">
                ...
                <div class="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md border border-slate-100 rounded-2xl p-3 text-[11px] text-slate-600 text-center z-20 shadow-md">
                    📍 Klik pin pada peta untuk melihat detail spesifik gudang.
                </div>
            </div>
        </div>
