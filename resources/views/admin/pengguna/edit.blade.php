...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-slate-500 text-sm">Perbarui informasi dan role pengguna.</p>
        </div>
    </div>
        ...
        <form action="{{ route('pengguna.update', $user->id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-[#001842] focus:ring-[#001842] transition-colors" required>
                    @error('name')<p class="mt-1 text-sm text-rose-500">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Email</label>
                    <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-[#001842] focus:ring-[#001842] transition-colors" required>
            ...
            <div class="border-t border-slate-100 pt-6 mt-6">
                <p class="text-sm text-amber-600 mb-4 bg-amber-50 px-4 py-2 rounded-lg inline-block font-medium">Biarkan kosong jika tidak ingin mengubah password.</p>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Password Baru</label>
                        <input type="password" name="password" class="w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-[#001842] focus:ring-[#001842] transition-colors">
                    ...
                    <div>
                        ...
                        <input type="password" name="password_confirmation" class="w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-[#001842] focus:ring-[#001842] transition-colors">
                    </div>
                </div>
            </div>
            ...
            <div class="pt-4 flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-3 rounded-xl bg-[#001842] text-white font-semibold hover:bg-[#002a70] transition-colors">Perbarui Pengguna</button>
            </div>
        </form>
    </div>
</div>
