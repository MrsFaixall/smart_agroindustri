...
<div class="mx-auto max-w-7xl space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-amber-900 via-orange-950 to-slate-900 p-6 md:p-8 rounded-3xl text-white shadow-xl shadow-slate-200/50 relative overflow-hidden">
        ...
        <div class="relative z-10 space-y-1">
            ...
            <h1 class="text-2xl md:text-3xl font-extrabold tracking-tight">Manajemen Harga Komoditas</h1>
            <p class="text-amber-100/80 text-sm max-w-xl">Monitor dan sesuaikan harga acuan pasaran kentang secara real-time.</p>
        </div>
        ...
        <div class="space-y-6 xl:col-span-3">
            <div class="overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-xl shadow-slate-100/60">
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-5">
                    <div>
                        <h2 class="font-bold text-lg text-slate-800">Daftar Harga Komoditas</h2>
                        <p class="text-xs text-slate-400">Harga acuan per kilogram</p>
                    </div>
                    <span class="rounded-xl bg-amber-50 border border-amber-200/60 px-3 py-1 text-[11px] font-bold text-amber-800 uppercase tracking-wider">{{ $summary['total'] }} KOMODITAS</span>
                </div>
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        @forelse($prices as $price)
                            <tr class="transition-colors hover:bg-slate-50/80">
                                ...
                                <td class="px-6 py-4 font-mono text-base font-extrabold text-amber-900">
                                    Rp {{ number_format($price->harga, 0, ',', '.') }}
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('atur-harga.edit', $price) }}" class="rounded-xl bg-blue-50 px-3.5 py-2 text-xs font-bold text-blue-700 hover:bg-blue-100 transition-all">Set Harga</a>
                                        <form action="{{ route('atur-harga.destroy', $price) }}" method="POST" onsubmit="return confirm(@js('Hapus harga '.($price->jenisKentang->nama_jenis ?? '').'?'))">
                                            @csrf 
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-16 text-center">
                                    ...
                                    <p class="mt-3 text-sm font-medium text-slate-500">Belum ada harga komoditas.</p>
