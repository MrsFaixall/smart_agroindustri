...
<div class="space-y-8">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-teal-900 via-emerald-950 to-slate-900 p-6 md:p-8 rounded-3xl text-white shadow-xl shadow-slate-200/50 relative overflow-hidden">
        ...
        <div class="relative z-10 space-y-1">
            ...
            <h1 class="text-2xl md:text-3xl font-extrabold tracking-tight">Riwayat Penjualan Panen</h1>
            <p class="text-teal-100/80 text-sm max-w-xl">Rekap riwayat hasil panen Anda yang telah dibeli oleh Koperasi.</p>
        </div>
    </div>

    <div class="overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-xl shadow-slate-100/60">
    <x-petani-table-filter placeholder="Cari data riwayat penjualan panen..." />
            ...
            <thead class="bg-slate-50/80 text-[11px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100">
                <tr>
                    ...
                    <th class="px-6 py-4">Pembeli (Koperasi)</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    <td class="px-6 py-4 text-xs font-semibold text-slate-500">{{ \Carbon\Carbon::parse($t->tanggal_pembelian)->translatedFormat('d M Y') }}</td>
                ...
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center text-slate-400">
                        <p class="text-sm font-medium">Belum ada riwayat penjualan panen.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    @if($transaksis->isNotEmpty())
        @include('partials.pagination', ['paginator' => $transaksis, 'label' => 'riwayat penjualan'])
    @endif
</div>
@endsection
