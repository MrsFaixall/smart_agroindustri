...
<div class="max-w-3xl mx-auto space-y-6">
    <!-- Header Section -->
    <div class="space-y-1">
        <h1 class="text-2xl font-bold text-slate-900">Edit Metode Pembayaran</h1>
        <p class="text-slate-500 text-sm">Perbarui informasi rekening bank, e-wallet, atau QRIS Anda.</p>
    </div>
    ...
    <!-- Form Card -->
    <form action="{{ route('metode-pembayaran.update', $method->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl p-8 shadow-sm border border-slate-200 border-t-4 border-t-[#001842] space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid gap-6 md:grid-cols-2">
            ...
            <div class="space-y-2 md:col-span-2">
                ...
                <select name="kategori" id="kategori" class="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none transition-all" required>
                    <option value="Transfer Bank" {{ (old('kategori') ?? $method->kategori) == 'Transfer Bank' ? 'selected' : '' }}>Transfer Bank</option>
                    <option value="E-Wallet" {{ (old('kategori') ?? $method->kategori) == 'E-Wallet' ? 'selected' : '' }}>E-Wallet (OVO, Gopay, Dana, dll)</option>
                    <option value="QRIS" {{ (old('kategori') ?? $method->kategori) == 'QRIS' ? 'selected' : '' }}>QRIS</option>
                </select>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="bank" id="bank" value="{{ old('bank', $method->bank) }}" placeholder="Contoh: BRI, Gopay, OVO" 
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="no_rekening" id="no_rekening" value="{{ old('no_rekening', $method->no_rekening) }}" placeholder="Contoh: 08123456789 / 12345678" 
                    class="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none transition-all" required>
            </div>

            <!-- QRIS Upload -->
            <div class="space-y-2">
                <label for="qr_image" class="text-sm font-bold text-slate-700">Upload QR Code Baru (Khusus QRIS)</label>
                ...
                <p class="text-xs text-slate-500">Biarkan kosong jika tidak ingin mengubah QR saat ini.</p>
                
                @if($method->qr_image)
                    <div class="mt-2">
                        <p class="text-xs text-slate-500 mb-1">QR Code Saat Ini:</p>
                        <img src="{{ asset('storage/' . $method->qr_image) }}" alt="QR Code" class="h-20 w-auto rounded border border-slate-200">
                    </div>
                @endif
            </div>
        </div>
