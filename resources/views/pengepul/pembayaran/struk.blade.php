...
<body>
    <div style="max-width: 80mm; width: 100%;">
        ...
        <div class="struk-card">
            ...
            <div class="row">
                <span>No. TRX:</span>
                <span class="bold">{{ $payment->pembelian->kode_trx ?? ('TRX-' . $payment->pembelian_id) }}</span>
            </div>
            <div class="row">
                <span>Tanggal:</span>
            ...
            <div class="row">
                <span>Petani:</span>
                <span class="bold">{{ substr($payment->pembelian->petani->name ?? 'Petani', 0, 15) }}</span>
            </div>
            ...
            <div class="row">
                <span>{{ $payment->pembelian->jenisKentang->nama_jenis ?? 'Kentang' }}</span>
            </div>
            <div class="row">
                <span>{{ number_format($payment->pembelian->jumlah_kg ?? 0, 2, ',', '.') }} Kg x Rp {{ number_format(($payment->pembelian->total_harga ?? 0) / max(1, $payment->pembelian->jumlah_kg ?? 1), 0, ',', '.') }}</span>
                <span class="bold">Rp {{ number_format($payment->pembelian->total_harga ?? 0, 0, ',', '.') }}</span>
            </div>
