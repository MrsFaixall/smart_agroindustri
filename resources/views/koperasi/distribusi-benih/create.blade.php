...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
        <a href="{{ route('distribusi-benih.index') }}" class="p-2.5 rounded-2xl bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 transition-all shadow-sm">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Catat Distribusi Benih (Koperasi -> Petani)</h1>
            <p class="text-xs text-slate-400">Penyaluran/penjualan benih dari Koperasi ke Petani.</p>
        </div>
    </div>
    ...
    <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-xl shadow-sm text-blue-700 text-sm">
        <strong>💡 Info Distribusi:</strong> Penyaluran benih dari form ini akan <strong>mengurangi</strong> stok benih Koperasi yang ada di menu <strong>Gudang & Stok</strong>.
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-slate-100/60 border border-slate-100 p-8 relative overflow-hidden">
        ...
        <form action="{{ route('distribusi-benih.store') }}" method="POST" class="space-y-6">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Petani Penerima <span class="text-rose-500">*</span></label>
                    <select name="petani_id" class="w-full select2" required>
                        <option value="">Pilih Petani</option>
                        @foreach($petanis as $petani)
                            <option value="{{ $petani->id }}">{{ $petani->name }} ({{ ucfirst($petani->role) }})</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div id="stok_status_card" class="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-1.5 transition-all">
                <div class="flex items-center justify-between text-xs">
                    <span class="font-bold text-slate-700 flex items-center gap-1.5">
                        <span>📦 Stok Koperasi Tersedia:</span>
                    </span>
                ...
                <p id="stok_status_detail" class="text-xs text-slate-500 italic">Silakan pilih jenis kentang untuk memeriksa ketersediaan stok koperasi.</p>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                <a href="{{ route('distribusi-benih.index') }}" class="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-all">Batal</a>
                <button type="submit" id="btn_submit_distribusi" class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold px-6 py-2.5 rounded-xl shadow-lg shadow-purple-600/30">Simpan Transaksi</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        const btnSubmit = document.getElementById('btn_submit_distribusi');
            ...
            $('#jenis_kentang_select').on('change', function() {
                ...
                if ($(this).val()) {
                    if (sisa > 0) {
                        ...
                        stokStatusDetail.textContent = '✓ Stok Benih Koperasi mencukupi. Anda dapat melakukan distribusi.';
                        ...
                        stokStatusDetail.textContent = '⚠️ PERHATIAN: Stok benih ini di Koperasi kosong. Anda harus mencatat Pengadaan Benih terlebih dahulu.';
                    }
                } else {
                    ...
                    stokStatusDetail.textContent = 'Silakan pilih jenis kentang untuk memeriksa ketersediaan stok koperasi.';
                }
            });
        }
    });
