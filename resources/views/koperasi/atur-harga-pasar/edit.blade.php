...
<div class="max-w-xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Edit Harga Pasar Koperasi</h1>
            <p class="text-xs text-slate-400">Ubah detail nominal harga jual untuk konsumen/mitra.</p>
        </div>
    </div>
    ...
    <form action="{{ route('koperasi.atur-harga-pasar.update', $price->id) }}" method="POST" class="space-y-6 rounded-3xl bg-white p-8 shadow-xl shadow-slate-100/60 border border-slate-100 relative overflow-hidden">
        ...
        <div class="space-y-4">
            <div>
                ...
                <select name="jenis_kentang_id" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-slate-800 focus:border-emerald-500 transition-all outline-none" required>
                    @foreach($jenisKentangs as $jk)
                        <option value="{{ $jk->id }}" {{ $price->jenis_kentang_id == $jk->id ? 'selected' : '' }}>{{ $jk->nama_jenis }} ({{ ucfirst($jk->kategori) }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number" name="harga" value="{{ old('harga', $price->harga) }}" placeholder="Masukkan nominal harga jual" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-slate-800 focus:border-emerald-500 transition-all outline-none font-bold text-emerald-700" required>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button type="submit" class="rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-emerald-600/30 transition-all">Simpan Perubahan</button>
        </div>
    </form>
</div>
@endsection
