...
<div class="space-y-8">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-teal-900 via-emerald-950 to-slate-900 p-6 md:p-8 rounded-3xl text-white shadow-xl shadow-slate-200/50 relative overflow-hidden">
        ...
        <div class="relative z-10 space-y-1">
            ...
            <p class="text-teal-100/80 text-sm max-w-xl">Rekap seluruh pembayaran atas transaksi pembelian yang dilakukan.</p>
        </div>
    </div>
        ...
        <table class="w-full border-collapse text-left text-sm">
            <thead class="bg-slate-50/80 text-[11px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100">
                <tr>
                    ...
                    <th class="px-6 py-4">Tujuan Pembayaran</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4 font-bold text-slate-800">🧑‍🌾 {{ $t->pembelian->petani->name ?? '-' }}</td>
                    ...
                    <td class="px-6 py-4 text-center">
                        @if($t->status === 'lunas')
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-bold uppercase tracking-wider">✔ Berhasil</span>
                        @else
