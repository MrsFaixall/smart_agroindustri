...
<div class="space-y-8">
    ...
    <div class="bg-white p-4 rounded-3xl shadow-xl shadow-slate-100/60 border border-slate-100">
        <form action="{{ route('koperasi.pembayaran.distribusi') }}" method="GET" class="flex flex-col lg:flex-row items-stretch lg:items-center gap-3 w-full">
            ...
            <div class="flex items-center gap-2">
                ...
                @if(request('search') || request('period') || request('start_date') || request('end_date'))
                    <a href="{{ route('koperasi.pembayaran.distribusi') }}" class="inline-flex items-center px-3 py-2.5 text-xs font-semibold rounded-2xl text-slate-600 bg-slate-100 hover:bg-slate-200 transition-all">
                        Reset
                    </a>
                @endif
            </div>
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                    ...
                    Daftar Distribusi Benih (Tagihan)
                </h2>
                <p class="text-xs text-slate-400">Seluruh data transaksi distribusi yang perlu diproses atau telah dilunasi.</p>
            </div>
        </div>
                        ...
                        @forelse($transaksis as $transaksi)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                <td class="px-6 py-4 font-bold text-slate-900 font-mono">
                                    <span class="block font-mono text-indigo-950 font-extrabold text-sm">{{ $transaksi->kode_trx }}</span>
                                    <span class="block text-[11px] font-semibold text-slate-400 font-mono tracking-tight mt-0.5">Ref: TRX-{{ str_pad($transaksi->id, 3, '0', STR_PAD_LEFT) }}</span>
                                </td>
                                <td class="px-6 py-4 text-slate-600 font-medium text-xs">
                                    {{ \Carbon\Carbon::parse($transaksi->tanggal_distribusi)->translatedFormat('d M Y') }}
                                </td>
                                <td class="px-6 py-4">
                                    <span class="font-bold text-slate-800">{{ $transaksi->petani->name ?? '-' }}</span>
                                    <span class="block text-xs text-slate-400 mt-0.5">{{ $transaksi->jenisKentang->nama_jenis ?? '-' }}</span>
                                </td>
                                <td class="px-6 py-4 text-right font-semibold text-slate-700 font-mono">
                                    {{ number_format($transaksi->jumlah_kg, 2, ',', '.') }} Kg
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if($transaksi->status === 'lunas')
                                        <div class="inline-flex items-center gap-2">
                                            <a href="{{ route('koperasi.pembayaran.distribusi.invoice', $transaksi->id) }}" class="inline-flex items-center px-3 py-1.5 rounded-xl bg-blue-50 text-blue-700 text-xs font-bold hover:bg-blue-100 transition-colors">
                                                Invoice
                                            </a>
                                            <a href="{{ route('koperasi.pembayaran.distribusi.struk', $transaksi->id) }}" target="_blank" class="inline-flex items-center px-3 py-1.5 rounded-xl bg-emerald-50 text-emerald-800 text-xs font-bold hover:bg-emerald-100 transition-colors">
                                                Struk
                                            </a>
                                        </div>
                                    @else
                                        <a href="{{ route('koperasi.pembayaran.distribusi.create', ['distribusi_id' => $transaksi->id]) }}" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold shadow-md shadow-emerald-600/20 transition-all">
                                            Bayar Sekarang
                                        </a>
                                    @endif
                                </td>
        ...
        @if($transaksis->hasPages())
            <div class="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-4 rounded-3xl border border-slate-100 shadow-sm">
                <p class="text-xs text-slate-500 font-medium">
                    Menampilkan <span class="font-bold text-slate-800">{{ $transaksis->firstItem() ?? 0 }}</span> sampai <span class="font-bold text-slate-800">{{ $transaksis->lastItem() ?? 0 }}</span> dari <span class="font-bold text-slate-800">{{ $transaksis->total() }}</span> pembelian
                </p>
                <div>
                    {{ $transaksis->links() }}
                </div>
                        ...
                        @forelse($payments as $payment)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-6 py-4 font-bold text-slate-800">
                                    ...
                                    <span class="block text-[11px] font-semibold text-slate-400 font-mono tracking-tight mt-0.5">Ref: {{ $payment->distribusiBenih->kode_trx ?? ('TRX-' . $payment->distribusi_benih_id) }}</span>
                                </td>
                                <td class="px-6 py-4 font-bold text-slate-800">
                                    {{ $payment->distribusiBenih->petani->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="inline-flex items-center gap-2">
                                        <a href="{{ route('koperasi.pembayaran.distribusi.invoice', $payment->id) }}" class="inline-flex items-center px-3 py-1.5 rounded-xl bg-blue-50 text-blue-700 text-xs font-bold hover:bg-blue-100 transition-colors">
                                            Invoice
                                        </a>
                                        <a href="{{ route('koperasi.pembayaran.distribusi.struk', $payment->id) }}" target="_blank" class="inline-flex items-center px-3 py-1.5 rounded-xl bg-emerald-50 text-emerald-800 text-xs font-bold hover:bg-emerald-100 transition-colors">
                                            Struk
                                        </a>
                                    </div>
                                </td>
