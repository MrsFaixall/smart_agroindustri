...
<body>
    <div style="max-width: 80mm; width: 100%;">
        ...
        <div class="struk-card">
            ...
            <div class="row">
                <span>No. TRX:</span>
                <span class="bold">{{ $payment->distribusiBenih->kode_trx ?? ('TRX-' . $payment->distribusi_benih_id) }}</span>
            </div>
            <div class="row">
                <span>Tanggal:</span>
            ...
            <div class="row">
                <span>Koperasi:</span>
                <span class="bold">{{ substr($payment->distribusiBenih->koperasi->name ?? 'Koperasi', 0, 15) }}</span>
            </div>
            <div class="row">
                <span>Petani:</span>
                <span class="bold">{{ substr($payment->distribusiBenih->petani->name ?? 'Petani', 0, 15) }}</span>
            </div>
            ...
            <div class="row">
                <span>{{ $payment->distribusiBenih->jenisKentang->nama_jenis ?? 'Kentang' }}</span>
            </div>
            <div class="row">
                <span>{{ number_format($payment->distribusiBenih->jumlah_kg ?? 0, 2, ',', '.') }} Kg x Rp {{ number_format(($payment->distribusiBenih->total_harga ?? 0) / max(1, $payment->distribusiBenih->jumlah_kg ?? 1), 0, ',', '.') }}</span>
                <span class="bold">Rp {{ number_format($payment->distribusiBenih->total_harga ?? 0, 0, ',', '.') }}</span>
            </div>
