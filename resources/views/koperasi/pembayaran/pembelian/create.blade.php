...
<div class="max-w-2xl mx-auto space-y-6 pt-4">
    <div class="flex items-center gap-3">
        <a href="{{ route('pembayaran.index') }}" class="p-2.5 rounded-2xl bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 transition-all shadow-sm">
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-slate-100/60 border border-slate-100 overflow-hidden relative">
        ...
        <form action="{{ route('pembayaran.store') }}" method="POST" id="payment-form" class="p-8 space-y-6">
            @csrf
            ...
            <div class="space-y-5">
                <!-- Pembelian Select -->
                <div class="space-y-2">
                    <label for="pembelian_id" class="block text-sm font-semibold text-slate-700">Pilih Transaksi Pembelian</label>
                    <select name="pembelian_id" id="pembelian_id" 
                        ...
                        @forelse($pembelians as $pembelian)
                            <option value="{{ $pembelian->id }}" data-petani="{{ $pembelian->petani->name ?? 'N/A' }}" data-total="{{ $pembelian->total_harga }}" {{ old('pembelian_id', request('pembelian_id')) == $pembelian->id ? 'selected' : '' }}>
                                TRX-{{ str_pad($pembelian->id, 5, '0', STR_PAD_LEFT) }} — {{ $pembelian->petani->name ?? 'Petani' }} — Rp {{ number_format($pembelian->total_harga, 0, ',', '.') }} ({{ \Carbon\Carbon::parse($pembelian->tanggal_pembelian)->format('d M Y') }})
                            </option>
                        @empty
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                <a href="{{ route('pembayaran.index') }}" class="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-all">Batal</a>
