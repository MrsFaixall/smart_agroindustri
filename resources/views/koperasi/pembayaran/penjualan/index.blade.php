...
<div class="space-y-8">
    ...
    <div class="bg-white p-4 rounded-3xl shadow-xl shadow-slate-100/60 border border-slate-100">
        <form action="{{ route('koperasi.pembayaran.penjualan') }}" method="GET" class="flex flex-col lg:flex-row items-stretch lg:items-center gap-3 w-full">
            ...
            <div class="flex items-center gap-2">
                ...
                @if(request('search') || request('period') || request('start_date') || request('end_date'))
                    <a href="{{ route('koperasi.pembayaran.penjualan') }}" class="inline-flex items-center px-3 py-2.5 text-xs font-semibold rounded-2xl text-slate-600 bg-slate-100 hover:bg-slate-200 transition-all">
                        Reset
                    </a>
                @endif
            </div>
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                    ...
                    Daftar Penjualan Buah (Tagihan)
                </h2>
                <p class="text-xs text-slate-400">Seluruh data transaksi penjualan yang perlu diproses atau telah dilunasi.</p>
            </div>
        </div>
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if($transaksi->status === 'lunas')
                                        <div class="inline-flex items-center gap-2">
                                            <a href="{{ route('koperasi.pembayaran.penjualan.invoice', $transaksi->id) }}" class="inline-flex items-center px-3 py-1.5 rounded-xl bg-blue-50 text-blue-700 text-xs font-bold hover:bg-blue-100 transition-colors">
                                                Invoice
                                            </a>
                                            <a href="{{ route('koperasi.pembayaran.penjualan.struk', $transaksi->id) }}" target="_blank" class="inline-flex items-center px-3 py-1.5 rounded-xl bg-emerald-50 text-emerald-800 text-xs font-bold hover:bg-emerald-100 transition-colors">
                                                Struk
                                            </a>
                                        </div>
                                    @else
                                        <a href="{{ route('koperasi.pembayaran.penjualan.create', ['penjualan_id' => $transaksi->id]) }}" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold shadow-md shadow-emerald-600/20 transition-all">
                                            Bayar Sekarang
                                        </a>
                                    @endif
                                </td>
                        ...
                        @forelse($payments as $payment)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-6 py-4 font-bold text-slate-800">
                                    ...
                                    <span class="block text-[11px] font-semibold text-slate-400 font-mono tracking-tight mt-0.5">Ref: {{ $payment->penjualanBuah->kode_trx ?? ('TRX-' . $payment->penjualan_buah_id) }}</span>
                                </td>
                                <td class="px-6 py-4 font-bold text-slate-800">
                                    {{ $payment->penjualanBuah->pembeli->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="inline-flex items-center gap-2">
                                        <a href="{{ route('koperasi.pembayaran.penjualan.invoice', $payment->id) }}" class="inline-flex items-center px-3 py-1.5 rounded-xl bg-blue-50 text-blue-700 text-xs font-bold hover:bg-blue-100 transition-colors">
                                            Invoice
                                        </a>
                                        <a href="{{ route('koperasi.pembayaran.penjualan.struk', $payment->id) }}" target="_blank" class="inline-flex items-center px-3 py-1.5 rounded-xl bg-emerald-50 text-emerald-800 text-xs font-bold hover:bg-emerald-100 transition-colors">
                                            Struk
                                        </a>
                                    </div>
                                </td>
