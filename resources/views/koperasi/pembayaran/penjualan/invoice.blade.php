...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 print:hidden">
        <a href="{{ route('koperasi.pembayaran.penjualan') }}" class="p-2.5 rounded-2xl bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 transition-all shadow-sm flex items-center gap-2 text-sm font-semibold">
            <x-heroicon-o-arrow-left class="w-4 h-4"/> Kembali ke Daftar Transaksi
        </a>
        <div class="flex items-center gap-3">
            <a href="{{ route('koperasi.pembayaran.penjualan.struk', $payment->id) }}" target="_blank" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-700 shadow-sm hover:bg-slate-50 transition-colors">
                🖨️ Cetak Struk (80mm)
            </a>
    ...
    <div id="invoice-printable" class="bg-white rounded-3xl border border-slate-100 p-8 md:p-12 shadow-2xl shadow-slate-200/50 relative overflow-hidden print:shadow-none print:border-none print:p-0">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-100 pb-8 gap-6 pt-2">
            ...
            <div class="text-left md:text-right">
                ...
                <p class="text-xs text-slate-400 font-semibold mt-0.5">Ref TRX: {{ $payment->penjualanBuah->kode_trx ?? ('TRX-' . $payment->penjualan_buah_id) }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 py-8 border-b border-slate-100">
            ...
            <div class="space-y-1">
                ...
                <h3 class="text-base font-extrabold text-slate-800">{{ $payment->penjualanBuah->pembeli->name ?? 'Petani' }}</h3>
            ...
            <div class="overflow-x-auto rounded-2xl border border-slate-100">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-slate-100 font-medium">
                        <tr>
                            <td class="px-6 py-4 font-mono font-bold text-indigo-950">#{{ $payment->penjualan_buah_id }}</td>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
            ...
            <div class="space-y-3">
                ...
                @if(($payment->penjualanBuah->total_harga ?? 0) - $payment->jumlah_bayar > 0)
                <div class="flex justify-between items-center text-xs text-amber-800 bg-amber-50/80 p-3 rounded-2xl border border-amber-200/60">
                    ...
                    <span class="font-bold font-mono">Rp {{ number_format(($payment->penjualanBuah->total_harga ?? 0) - $payment->jumlah_bayar, 0, ',', '.') }}</span>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="mt-12 pt-8 border-t border-slate-100 grid grid-cols-2 gap-8 text-center text-xs text-slate-500">
            <div>
                ...
                <p class="font-bold text-slate-800 uppercase">( {{ $payment->penjualanBuah->pembeli->name ?? 'Petani' }} )</p>
            </div>
            <div>
