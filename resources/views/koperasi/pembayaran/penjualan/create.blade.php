...
<div class="max-w-2xl mx-auto space-y-6 pt-4">
    <div class="flex items-center gap-3">
        <a href="{{ route('koperasi.pembayaran.penjualan') }}" class="p-2.5 rounded-2xl bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 transition-all shadow-sm">
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-slate-100/60 border border-slate-100 overflow-hidden relative">
        ...
        <form action="{{ route('koperasi.pembayaran.penjualan.store') }}" method="POST" id="payment-form" class="p-8 space-y-6">
            @csrf
                ...
                <div class="space-y-2">
                    ...
                    <select name="penjualan_buah_id" id="penjualan_buah_id" 
                        ...
                        @forelse($transaksis as $transaksi)
                            <option value="{{ $transaksi->id }}" data-petani="{{ $transaksi->petani->name ?? 'N/A' }}" data-total="{{ $transaksi->total_harga }}" {{ old('penjualan_buah_id', request('penjualan_buah_id')) == $transaksi->id ? 'selected' : '' }}>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                <a href="{{ route('koperasi.pembayaran.penjualan') }}" class="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-all">Batal</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const pembelianSelect = document.getElementById('penjualan_buah_id');
        ...
        if (typeof $ !== 'undefined' && $.fn.select2) {
            ...
            $('#penjualan_buah_id').on('change', updateSummary);
        }
