...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-indigo-950 via-slate-900 to-indigo-950 p-6 md:p-8 rounded-3xl text-white shadow-xl shadow-slate-200/50 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <a href="{{ route('koperasi.stok-koperasi.create') }}"
    ...
    <div class="w-full bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-xl shadow-slate-100/60">
        ...
        <div class="overflow-x-auto w-full">
            <table class="w-full text-sm text-left whitespace-nowrap lg:whitespace-normal">
            <thead class="bg-slate-50/80 text-[11px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100">
                <tr>
                    ...
                    <th class="px-6 py-4">Alokasi PT CAMP</th>
                    <th class="px-6 py-4">Alokasi Konsumen</th>                    <th class="px-6 py-4 text-right">Total Fisik (Kg)</th>
            ...
            @forelse($stoks as $stok)
                @php
                    $alokasiCamp = $stok->alokasi_pt_camp ?? 0;
                    $alokasiKonsumen = $stok->alokasi_konsumen ?? 0;
                    $totalAlokasi = $alokasiCamp + $alokasiKonsumen;
                    $sisaFisik = max(0, $stok->jumlah_stok - $totalAlokasi);
                ...
                <tr class="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <div class="font-bold text-indigo-700 font-mono text-base flex items-center gap-1.5">
                            <span>🏭</span> {{ number_format($alokasiCamp, 0, ',', '.') }} Kg
                        </div>
                        <span class="text-[10px] text-slate-400">Untuk PT CAMP</span>
                    </td>

                    <td class="px-6 py-4">
                        <div class="font-bold text-teal-700 font-mono text-base flex items-center gap-1.5">
                            <span>🛒</span> {{ number_format($alokasiKonsumen, 0, ',', '.') }} Kg
                        </div>
                        <span class="text-[10px] text-slate-400">Untuk Konsumen</span>
                    </td>

                    <td class="px-6 py-4 text-right">
                        <div class="font-extrabold text-slate-700 font-mono text-base">
                            {{ number_format($stok->jumlah_stok, 0, ',', '.') }} Kg
                        </div>
                        <span class="text-[10px] text-slate-400">Total di gudang</span>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex justify-end gap-2">
                            <a href="#" onclick="openAlokasiModal('{{ $stok->id }}', '{{ $stok->jumlah_stok }}', '{{ $alokasiCamp }}', '{{ $alokasiKonsumen }}'); event.preventDefault();"
                               class="rounded-xl bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-700 hover:bg-indigo-100 transition-colors">
                                ⚙️ Sesuaikan Alokasi
                            </a>

                            <form action="{{ route('koperasi.stok-koperasi.destroy', $stok->id) }}"
                                  method="POST"
...
<script>
function openAlokasiModal(id, total, camp, konsumen) {
    document.getElementById('modal_alokasi').classList.remove('hidden');
    document.getElementById('form_alokasi').action = `/koperasi/stok-koperasi/${id}`;
    document.getElementById('total_stok_info').textContent = parseFloat(total).toLocaleString('id-ID');
    document.getElementById('input_camp').value = camp;
    document.getElementById('input_konsumen').value = konsumen;
}

function closeAlokasiModal() {
    document.getElementById('modal_alokasi').classList.add('hidden');
}
</script>

<!-- Modal Alokasi -->
<div id="modal_alokasi" class="fixed inset-0 z-[100] hidden">
    <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" onclick="closeAlokasiModal()"></div>
    <div class="fixed inset-0 z-10 flex items-center justify-center overflow-y-auto p-4">
        <div class="relative w-full max-w-md transform overflow-hidden rounded-3xl bg-white p-6 text-left shadow-2xl transition-all border border-slate-100">
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h3 class="text-xl font-extrabold text-slate-800 tracking-tight">Sesuaikan Alokasi Penjualan</h3>
                    <p class="text-xs font-medium text-slate-500 mt-1">Stok Fisik Tersedia: <span id="total_stok_info" class="font-bold text-slate-800"></span> Kg</p>
                </div>
                <button type="button" onclick="closeAlokasiModal()" class="rounded-full p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors">
            ...
            <form id="form_alokasi" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1">Alokasi untuk PT CAMP (Kg)</label>
                    <input type="number" step="0.01" min="0" name="alokasi_pt_camp" id="input_camp" required
                           class="block w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm font-medium">
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1">Alokasi untuk Konsumen (Kg)</label>
                    <input type="number" step="0.01" min="0" name="alokasi_konsumen" id="input_konsumen" required
                           class="block w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm font-medium">
                </div>

                <div class="mt-6 flex justify-end gap-3 pt-4 border-t border-slate-100">
                    <button type="button" onclick="closeAlokasiModal()"
                            class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm hover:bg-slate-50 transition-colors">
                        Batal
                    </button>
                    <button type="submit"
                            class="rounded-xl bg-indigo-600 px-6 py-2 text-sm font-bold text-white shadow-sm hover:bg-indigo-700 transition-colors">
                        Simpan Alokasi
                    </button>
                </div>
            </form>
