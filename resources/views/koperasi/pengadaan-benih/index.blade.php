...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-purple-900 via-indigo-950 to-slate-900 p-6 md:p-8 rounded-3xl text-white shadow-xl shadow-slate-200/50 relative overflow-hidden">
        ...
        <div class="relative z-10 space-y-1">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/20 border border-purple-500/30 text-purple-300 text-xs font-semibold mb-1 backdrop-blur-md">
                ...
                <span>Penyedia & Pemasok Benih (PT Champ)</span>
            </div>
            <h1 class="text-2xl md:text-3xl font-extrabold tracking-tight">Pengadaan Benih</h1>
            <p class="text-purple-100/80 text-sm max-w-xl">Kelola rekapitulasi pembelian benih terstandar dari PT Champ ke Koperasi.</p>
        </div>
        <div class="relative z-10">
            <a href="{{ route('pengadaan-benih.create') }}" class="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white px-5 py-2.5 rounded-xl flex items-center gap-2 transition-all duration-200 text-sm font-bold shadow-lg shadow-purple-600/30 transform hover:-translate-y-0.5">
                <span class="text-lg leading-none">+</span> Tambah Pengadaan
            </a>
        </div>
    </div>
    ...
    <div class="bg-white p-4 rounded-3xl shadow-xl shadow-slate-100/60 border border-slate-100">
        <form action="{{ route('pengadaan-benih.index') }}" method="GET" class="flex flex-col lg:flex-row items-stretch lg:items-center gap-3 w-full">
            <div class="relative flex-1 min-w-[180px]">
                ...
                <input type="text" name="search" value="{{ request('search') }}" 
                    ...
                    placeholder="Cari varietas benih atau nama mitra...">
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                @if(request('search') || request('start_date') || request('end_date'))
                    <a href="{{ route('pengadaan-benih.index') }}" class="inline-flex items-center px-4 py-2.5 text-xs font-semibold rounded-2xl text-slate-600 bg-slate-100 hover:bg-slate-200 transition-all">
                        Reset
                    </a>
                @endif
            </div>
        ...
        <table class="w-full border-collapse text-left text-sm">
            <thead class="bg-slate-50/80 text-[11px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100">
                <tr>
                    ...
                    <th class="px-6 py-4">Mitra (PT Champ)</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td class="px-6 py-4 text-sm font-semibold text-slate-600">🏢 {{ $t->mitra->name ?? '-' }}</td>
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center text-slate-400">
                        <p class="text-sm font-medium">Belum ada catatan transaksi pengadaan benih.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    @if($transaksis->isNotEmpty())
        @include('partials.pagination', ['paginator' => $transaksis, 'label' => 'transaksi pengadaan'])
    @endif
</div>
@endsection
