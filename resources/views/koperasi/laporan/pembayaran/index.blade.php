...
<div class="space-y-6 md:space-y-8 print:m-0 print:p-0">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 print:hidden">
        <div>
            ...
            <h1 class="text-2xl md:text-3xl font-extrabold text-slate-900 tracking-tight">Laporan Pembayaran Keluar</h1>
            <p class="text-sm text-slate-500 mt-1">Pembayaran dana dari Koperasi ke Petani.</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <a href="{{ route('koperasi.laporan.pembayaran.export', ['month' => $month, 'year' => $year]) }}" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-[#001842] text-white font-semibold hover:bg-blue-900 transition shadow-lg shadow-blue-900/20">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 md:p-6 print:hidden">
        <form action="{{ route('koperasi.laporan.pembayaran') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end">
    ...
    <div class="hidden print:block text-center mb-8 border-b-2 border-slate-800 pb-4">
        <h1 class="text-2xl font-bold uppercase text-slate-900">Laporan Pembayaran Keluar</h1>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead class="bg-slate-50 text-[11px] font-bold uppercase tracking-wider text-slate-500 border-b border-slate-100">
                    <tr>
                        <th class="px-6 py-4">Tanggal</th><th class="px-6 py-4">Referensi</th><th class="px-6 py-4">Penerima</th><th class="px-6 py-4 text-right">Jumlah Bayar</th><th class="px-6 py-4 text-center">Status</th>
                    </tr>
                </thead>
    ...
    <td class="px-6 py-4">{{ $item->pembelian->petani->name ?? '-' }}</td>
