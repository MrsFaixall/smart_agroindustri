        ...
        <form action="{{ route('pembelian.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="koperasi_id" id="koperasi_select" class="w-full rounded-2xl border-slate-200 px-4 py-3 select2" required {{ auth()->user()->role === 'koperasi' || $koperasis->count() == 1 ? 'disabled' : '' }}>
                        ...
                        @foreach($koperasis as $koperasi)
                            <option value="{{ $koperasi->id }}" {{ (old('koperasi_id') == $koperasi->id || auth()->id() == $koperasi->id || $koperasis->count() == 1) ? 'selected' : '' }}>🏢 {{ $koperasi->name }}</option>
                        @endforeach
                    </select>
                    @if(auth()->user()->role === 'koperasi' || $koperasis->count() == 1)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="jenis_kentang_id" id="jenis_kentang_select" class="w-full rounded-2xl border-slate-200 px-4 py-3 select2" required disabled>
...
<script>
    ...
    window.oldMetodePembayaran = "{{ old('metode_pembayaran_id') }}";
    window.oldJenisKentang = "{{ old('jenis_kentang_id') }}";
</script>
<script>
