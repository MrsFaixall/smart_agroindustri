...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-slate-400">Penjualan stok komoditas kentang Koperasi ke Mitra atau Konsumen.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-slate-100/60 border border-slate-100 p-8 relative overflow-hidden">
        ...
        <form action="{{ route('penjualan-buah.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                <a href="{{ route('penjualan-buah.index') }}" class="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-all">Batal</a>
                <button type="submit" id="btn_submit_penjualan" class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold px-6 py-2.5 rounded-xl shadow-lg shadow-purple-600/30">Simpan Transaksi</button>
            </div>
        </form>
    </div>
</div>
            ...
            $('#jenis_kentang_select').on('change', function() {
                ...
                if ($(this).val()) {
                    if (sisa > 0) {
                        ...
                        stokStatusDetail.textContent = '✓ Stok Hasil Panen Koperasi mencukupi. Anda dapat melakukan penjualan.';
                        ...
                        stokStatusDetail.textContent = '⚠️ PERHATIAN: Stok hasil panen ini di Koperasi kosong. Anda harus melakukan pembelian hasil panen dari Petani terlebih dahulu.';
                    }
                } else {
